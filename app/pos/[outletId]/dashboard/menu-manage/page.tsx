...
export default function MenuManagePage() {
  ...
  // Existing unique categories extract karne ke liye
  ...
  const triggerOfflineQueueSync = async () => {
    const savedQueue = localStorage.getItem(`zapped_offline_menu_queue_${outletId}`);
    ...
    if (remaining.length === 0) localStorage.removeItem(`zapped_offline_menu_queue_${outletId}`);
    else localStorage.setItem(`zapped_offline_menu_queue_${outletId}`, JSON.stringify(remaining));
  ...
  const fetchMenu = async () => {
    ...
    try {
      ...
      // Sync to Dexie LocalDB for offline use
      await localDB.menuItems.clear(); // Using clear & put for a clean state sync
      if (menuData.length > 0) {
        await localDB.menuItems.bulkPut(menuData);
      }
      
      // Default category set karne ke liye agar list khali na ho
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Naya 4-digit random number generate kar rahe hain (e.g. 5291)
    ...
    // Agar naya item ban raha hai toh POST me ItemId bhi bhejenge
    ...
    if (!navigator.onLine) {
      // OFFLINE HANDLING
      const queue = JSON.parse(localStorage.getItem(`zapped_offline_menu_queue_${outletId}`) || "[]");
      queue.push({ method, body: bodyData });
      localStorage.setItem(`zapped_offline_menu_queue_${outletId}`, JSON.stringify(queue));
      
      // Optimistic update in LocalDB
      await localDB.menuItems.put({ ...bodyData, price: parseFloat(payload.finalPrice), isActive: true, isDeleted: false });
  ...
  // Reverse GST math calculation display
