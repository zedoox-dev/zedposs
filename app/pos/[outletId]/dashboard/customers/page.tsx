...
export default function CustomersCRMPage() {
  ...
  // 🔥 UPDATED: Advanced Marketing Template States
  ...
  useEffect(() => {
    ...
    const savedPromo = localStorage.getItem(`crm_template_promo_${outletId}`);
    if (savedPromo) setTemplatePromo(savedPromo);
    const savedCredit = localStorage.getItem(`crm_template_credit_${outletId}`);
    if (savedCredit) setTemplateCredit(savedCredit);
    const savedRedeem = localStorage.getItem(`crm_template_redeem_${outletId}`);
  ...
  const triggerOfflineQueueSync = async () => {
    const savedQueue = localStorage.getItem(`zapped_offline_crm_queue_${outletId}`);
    ...
    if (remaining.length === 0) localStorage.removeItem(`zapped_offline_crm_queue_${outletId}`);
    else localStorage.setItem(`zapped_offline_crm_queue_${outletId}`, JSON.stringify(remaining));
  ...
  const fetchCustomers = async () => {
    ...
    if (!navigator.onLine) {
      try {
        const localCustomers = await localDB.customers.where('tenantId').equals(tenantId).toArray();
    ...
    try {
      ...
      if (crmData.length > 0) {
        // Prepare for offline sync by mapping fields required by Dexie schema
        const cacheData = crmData.map(c => ({ ...c, tenantId }));
  ...
  const saveTemplates = () => {
      localStorage.setItem(`crm_template_promo_${outletId}`, templatePromo);
      localStorage.setItem(`crm_template_credit_${outletId}`, templateCredit);
      localStorage.setItem(`crm_template_redeem_${outletId}`, templateRedeem);
  ...
  const handleAddCustomer = async (e: React.FormEvent) => {
    ...
    if (!navigator.onLine) {
      const queue = JSON.parse(localStorage.getItem(`zapped_offline_crm_queue_${outletId}`) || "[]");
      queue.push({ method: "POST", body: payload });
      localStorage.setItem(`zapped_offline_crm_queue_${outletId}`, JSON.stringify(queue));
      
      // Optimistic offline creation
      const fakeId = `off-${Date.now()}`;
      await localDB.customers.put({ id: fakeId, ...payload, loyaltyPoints: 50, totalVisits: 0, totalSpend: 0 });
  ...
  // --- 👑 PRO FEATURE: VIP TIER CALCULATION ENGINE ---
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden print:h-auto print:overflow-visible print:bg-white">
            ...
            <div className="w-full lg:w-1/3 flex flex-col gap-6 h-full overflow-hidden">
              <div className="bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl p-6 shadow-xl relative overflow-hidden shrink-0 text-white">
                ...
                <div className="space-y-2 text-[10px] font-bold">
                  {/* 🔥 FIXED: Text updated to 10 Points = Rs 1 */}
            ...
            <form onSubmit={handleRedeemPoints} className="space-y-4 text-left">
              <div>
                ...
                <p className="text-[9px] text-center text-slate-400 font-bold uppercase mt-1.5">
                  {/* 🔥 FIXED: UI reflects 10 points = Rs 1 */}
      ...
      {/* 🖨️ FIXED: Conditional rendering so it ONLY prints the ACTIVE tab. */}
