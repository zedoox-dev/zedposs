...
export default function ItemSalesPage() {
  ...
  const fetchItemStats = async () => {
    ...
    if (!navigator.onLine) {
      const cached = localStorage.getItem(`zap_item_sales_${secureOutletId}_${dateFilter}`);
      if (cached) {
    ...
    try {
      ...
      if (json.success) {
        setData(json);
        localStorage.setItem(`zap_item_sales_${secureOutletId}_${dateFilter}`, JSON.stringify(json));
      }
    } catch (err) {
