...
export default function KitchenKOTPage() {
  ...
  // Live Orders laane ka function (Tenant Secured)
  const fetchOrders = () => {
    if (!session?.user || !navigator.onLine) return;
    const tenantId = (session.user as any).tenantId;

    fetch(`/api/kitchen?outletId=${outletId}&tenantId=${tenantId}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    if (session?.user) {
      fetchOrders();
      // Har 10 second me kitchen screen apne aap refresh hogi naye orders ke liye
  ...
  const markAsReady = async (orderId: string) => {
    ...
    try {
      ...
      // Agar fail hua toh wapas refresh maar do
      fetchOrders(); 
    }
  };

  // Data Filtering for Tabs
  // Live: Completed/Paid, FIFO (Pehle aaya, pehle dikhega)
  const liveOrders = allOrders.filter(o => o.status === "COMPLETED").reverse(); 
  
  // Prepared: Served, LIFO (Abhi jo ready hua wo pehle dikhega)
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {displayOrders.map((order) => {
              ...
              // Only RED if it is LIVE and older than 10 mins
              ...
              return (
                <div key={order.id} className={`bg-white rounded-2xl shadow-md border-t-8 p-5 flex flex-col transition-all ${
                  ...
                  {/* Action Button (Only visible in LIVE tab) */}
