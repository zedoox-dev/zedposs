...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleBillSearch = (e: React.FormEvent) => {
    ...
    if (searchBillNo.trim() !== "") {
      ...
      setSearchBillNo(""); // Search karne ke baad upar se clear kar do
    }
  };
  ...
  const currentUserName = session.user.name ? session.user.name.toUpperCase() : "SECURE USER";
  const currentUserOutlet = (session.user as any).outletId || outletId;
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 overflow-hidden relative font-sans">
      ...
      {/* 👑 WINDOW CONTROL BADGE */}
      <div className="h-7 bg-slate-950 flex items-center justify-between px-4 select-none z-50 shrink-0">
        <div className="text-[10px] font-black tracking-widest uppercase text-slate-400 flex items-center">
          <span className={`${isOnline ? 'text-emerald-500' : 'text-red-500'} mr-2 animate-pulse`}>●</span> 
          {currentUserName} 
          <span className="opacity-40 ml-2 hidden sm:inline">| SECURE ID: {currentUserOutlet}</span>
        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="bg-red-500/10 p-6 flex flex-col items-center text-center border-b border-slate-800">
              ...
              <p className="text-xs text-slate-400 font-bold mt-2 leading-relaxed">
                You are safe-closing terminal workspace <span className="text-white">[{currentUserName}]</span>. Active order states will reset securely.
              </p>
            </div>
