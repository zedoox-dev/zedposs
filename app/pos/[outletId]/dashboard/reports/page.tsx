...
export default function ReportsPage() {
  ...
  const fetchReportStats = async () => {
    ...
    if (!navigator.onLine) {
      const cached = localStorage.getItem(`zap_last_report_${secureOutletId}_${dateFilter}`);
      if (cached) {
    ...
    try {
      ...
      if (data.success) {
        setReportData(data);
        localStorage.setItem(`zap_last_report_${secureOutletId}_${dateFilter}`, JSON.stringify(data)); 
      }
    } catch (err) {
