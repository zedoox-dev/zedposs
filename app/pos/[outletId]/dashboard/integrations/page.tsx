...
export default function IntegrationHubPage() {
  ...
  // --- Real DB States (Expanded with Toing, Club & Scheduling) ---
  ...
  // --- Initial Data Fetch from Real DB Endpoints ---
  const loadDashboardData = async () => {
    ...
    setIsLoading(true);
    try {
      // 1. Fetch Real POS Base Menu Items from Inventory (Finished Goods)
      const invRes = await fetch(`/api/inventory?outletId=${outletId}&tenantId=${tenantId}`);
      if (invRes.ok) {
      ...
      // 2. Fetch Aggregator Platform Settings & Mappings from Integration DB
      const intRes = await fetch(`/api/integrations?outletId=${outletId}&tenantId=${tenantId}`);
      if (intRes.ok) {
        const intData = await intRes.json();
        
        if (intData.platforms) {
          // Merge DB status with UI styling config safely
          setPlatforms(prev => {
            const updated = { ...prev };
      ...
      // 3. Fetch Live Webhook Logs from Database
      const logsRes = await fetch(`/api/integrations/logs?outletId=${outletId}`);
      if (logsRes.ok) {
  ...
  const togglePlatformState = async (platformKey: keyof typeof platforms, keyToToggle: "isStoreOnline" | "autoAccept" | "menuSync") => {
    ...
    try {
      // Use the generic mapping endpoint to save state changes
      ...
      if (keyToToggle === "isStoreOnline") {
        ...
        loadDashboardData(); // Refresh logs silent update
      }
    } catch (e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetch("/api/integrations/logs", {
          ...
          body: JSON.stringify({ outletId, platform: "SYSTEM", event: "MASTER_SYNC", status: "SUCCESS", details: "Deep configuration and catalog pushed to remote APIs." })
        });
        loadDashboardData();
      } else {
  ...
  const triggerForceSync = async () => {
    ...
    setIsLoading(true);
    try {
      // Mocking Force Sync execution time
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      await fetch("/api/integrations/logs", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ outletId, platform: "SYSTEM", event: "FORCE_SYNC", status: "SUCCESS", details: "Manual force sync completed for all active channels." })
      });
