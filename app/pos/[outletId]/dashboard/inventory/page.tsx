...
export default function InventoryAndPurchaseERP() {
  ...
  // --- Inline Custom Security Authentication Modals ---
  ...
  const triggerOfflineQueueSync = async () => {
    // Basic offline sync placeholder for future expansion (Since logic here is highly complex)
  ...
  const fetchInventoryAndProduction = async () => {
    ...
    try {
      // 🔥 THE FULL DB SYNC 
      const res = await fetch(`/api/inventory?outletId=${outletId}&tenantId=${tenantId}`);
      ...
      // Yield History Fetch
      let prodUrl = `/api/production?outletId=${outletId}&tenantId=${tenantId}&date=${dateFilter}`;
  ...
  // --- Legacy / Unconnected features kept as UI ---
  const handleDeleteSKU = async (itemId: string, itemName: string) => {
    ...
    if (adminPasswordInput === realTerminalSavedPass) {
      setLoading(true);
      // Would call DELETE API here in future
  ...
  const handlePayOutstandingDues = () => {
    // Would connect to PUT /api/inventory in future
    alert("💸 Vendor Ledger Outstandings Settled! (Local UI Only)");
    setShowPayDueModal(false); setPayDueAmount("");
  };

  // --- HOISTED HIGHEST LEVEL ANALYTICS MATHEMATICAL ENGINES ---
  ...
  // --- 🔥 MOBILE VIEW OVERHAUL (100% VISIBLE SUBMIT BUTTON) ---
  if (isMobileMode) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden print:h-auto print:overflow-visible print:bg-white">
      ...
      {/* Delete Authorization Securty Lock Modal (RESTORED FIX) */}
