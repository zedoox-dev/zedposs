...
export default function OrdersHistoryPage() {
  ...
  // 🔥 THE GLOBAL SEARCH SYNC: Reads 'search' query from URL (Navbar se aane wali search)
  ...
  useEffect(() => {
    ...
    // Auto sync search input if URL changes (Layout Navbar Search Link)
  ...
  const fetchOrders = async () => {
    ...
    // 🔥 OFFLINE FETCHING (Reads from Dexie localDB)
    if (!navigator.onLine) {
      try {
        const offlineOrders = await localDB.orders.where('outletId').equals(outletId).reverse().toArray();
    ...
    // ONLINE FETCHING
    let url = `/api/orders?outletId=${outletId}&date=${dateFilter}`;
    ...
    try {
      ...
      if (fetchedOrders.length > 0) {
        // Bulk Put ensures offline data stays updated with latest status
  ...
  const displayedOrders = orders.filter(order => {
    ...
    // 🔥 FIXED: Status filtering now reliably accounts for paymentMode being COMPLEMENTARY
  ...
  // 🔥 FIXED: Badge directly checks paymentMode properly to tag complementary bills.
  ...
  return (
    <div className="flex h-full relative overflow-hidden bg-slate-50 print:overflow-visible">
      ...
      {/* ---------- MAIN DASHBOARD LAYOUT SCREEN (Hides on print) ---------- */}
      <div className="flex h-full w-full print:hidden">
        
        {/* LEFT COMPONENT: INVOICES GRID */}
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {displayedOrders.length === 0 ? (
                      ...
                      <tr key={order.id} className={`hover:bg-orange-50/50 transition-colors cursor-pointer ${selectedOrder?.id === order.id ? 'bg-orange-50' : ''}`} onClick={() => setSelectedOrder(order)}>
                        ...
                        {/* 🔥 FIXED: List displays 0.00 properly if it's a complementary bill */}
                        <td className="p-4 font-mono font-black text-slate-900">
                          ₹{(order.isComplementary || order.paymentMode === "COMPLEMENTARY") ? "0.00" : Number(order.totalAmount).toFixed(2)}
                        </td>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-6">
                ...
                <div className="bg-orange-50/50 p-4 rounded-xl border border-orange-100 space-y-2">
                  ...
                  <div className="border-t border-orange-200 pt-2 mt-2 flex justify-between items-end">
                    ...
                    {/* 🔥 FIXED: Inside Details panel too, so it stays perfectly synced with outside list */}
              ...
              {selectedOrder.status !== "CANCELLED" && (
                <div className="p-5 border-t border-slate-100 bg-white grid grid-cols-2 gap-3 shrink-0">
                  <button 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setPinModal({ isOpen: true, action: "CANCEL" });
                    }} 
                  ...
                  <button 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setPinModal({ isOpen: true, action: "REPRINT" });
                    }} 
        ...
        {lastReprintOrder && (
          <div className="w-full bg-white text-black p-1 flex flex-col items-center">
            ...
            <div className="w-full flex justify-between font-black text-[13px] border-b border-solid border-black pb-1 px-1">
              ...
              {/* 🔥 FIXED: Ensuring print receipt matches 0.00 for complementary */}
