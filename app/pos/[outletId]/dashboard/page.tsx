...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    const loadMenu = async () => {
      try {
        if (navigator.onLine) {
          // Online: Fetch from Main DB and update Local DB
          ...
          // Offline: Fetch straight from Local DB (Zero latency)
          const localMenuData = await localDB.menuItems.where('tenantId').equals(tenantId).toArray();
  ...
  const handleCheckout = async (actionType: "PRINT" | "EBILL" | "SAVE" | "HOLD") => {
    ...
    const payload = {
      tenantId, outletId, cart, totalAmount: grandTotal, paymentMode, orderType,
  ...
  return (
    <div className="flex h-full relative overflow-hidden print:overflow-visible">
      {/* EVERYTHING REMAINS EXACTLY THE SAME BELOW THIS POINT */}
