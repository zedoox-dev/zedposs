...
export default function MegaProductionERP() {
  ...
  const triggerOfflineQueueSync = async () => {
    const savedQueue = localStorage.getItem(`zapped_offline_prod_queue_${outletId}`);
    ...
    if (remaining.length === 0) localStorage.removeItem(`zapped_offline_prod_queue_${outletId}`);
    else localStorage.setItem(`zapped_offline_prod_queue_${outletId}`, JSON.stringify(remaining));
  ...
  const fetchCoreERP = async () => {
    ...
    if (!navigator.onLine) {
      try {
        const localMenus = await localDB.menuItems.where('tenantId').equals(tenantId).toArray();
        const localOrders = await localDB.orders.where('outletId').equals(outletId).toArray();
    ...
    let url = `/api/production?outletId=${outletId}&tenantId=${tenantId}&date=${dateFilter}`;
  ...
  const saveMapping = async () => {
    ...
    const payload = { action: "SAVE_MAPPING", outletId, tenantId: (session?.user as any)?.tenantId, mappingData: { menuItemId: mapItem, baseQty: "1", materials: adjustedMaterials } };

    if (!navigator.onLine) {
      const queue = JSON.parse(localStorage.getItem(`zapped_offline_prod_queue_${outletId}`) || "[]");
      queue.push(payload);
      localStorage.setItem(`zapped_offline_prod_queue_${outletId}`, JSON.stringify(queue));
  ...
  const submitProductionEntry = async () => {
    ...
    const payload = { 
      ...
      loggedByUserId: (session?.user as any)?.id,
  ...
  const executeHistoricalComparison = async () => {
    ...
    try {
      const bRes = await fetch(`/api/production?outletId=${outletId}&tenantId=${tenantId}&date=custom&startDate=${baseCompareDate}&endDate=${baseCompareDate}`);
      const bData = await bRes.json();
      const tRes = await fetch(`/api/production?outletId=${outletId}&tenantId=${tenantId}&date=custom&startDate=${targetCompareDate}&endDate=${targetCompareDate}`);
  ...
  // Helper for Global Total Revenue
    ...
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden print:h-auto print:overflow-visible print:bg-white">
      ...
      {loading ? (
        ...
        <div className="flex-1 p-6 overflow-y-auto custom-scrollbar print:p-0 print:overflow-visible print:h-auto">
          ...
          {/* TAB 2: MAPPING DETAILS (NO CHANGES, IT IS LOCKED PERFECTLY) */}
          {activeTab === "MAPPING" && (
          ...
          {/* ================= TAB 3: DAILY ENTRY (EXACT DEDUCTION UI TEXT ADDED) ================= */}
          {activeTab === "ENTRY" && (
            <div className="animate-in fade-in duration-200 flex flex-col lg:flex-row gap-6 print:hidden">
          ...
          {/* ================= TAB 5: RECENT HISTORY (4 NEW PRO FEATURES ADDED) ================= */}
          {activeTab === "HISTORY" && (
            <div className="animate-in fade-in duration-200 print:hidden">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <table className="w-full text-left border-collapse">
          ...
          {/* ================= TAB 7: ADVANCED COMPARE ANALYTICS (FIXED AND SUPERCHARGED) ================= */}
          {activeTab === "COMPARE" && (
      ...
      {/* ------------------- 🔥 ENTERPRISE LEDGER HARDWARE PRINT AREA (Strict Tab-Wise Logic Updated) ------------------- */}
